...
- **`List` Functor**: 리스트를 감싸고, 스칼라 값을 다루는 함수를 전체 리스트에 적용할 수 있게 합니다. 이는 파이썬 내장 `map()` 함수와 유사합니다.
    ...
    # List Functor: 리스트를 감쌈 (예시를 위해 값 추가)
    numbers_list = List.from_iterable([1, 2, 3])
    print(f"원래 List 값: {numbers_list}") # 출력: List(1, 2, 3)
    
    # Just로 감싼 함수를 List Functor에 apply
    # Applicative의 apply 메서드는 감싸진 함수를 감싸진 값에 적용합니다.
    # List Functor의 경우, 감싸진 함수가 Just 컨텍스트에 있으므로,
    # List(1, 2, 3)의 각 요소에 twice 함수가 적용됩니다.
    wrapped_twice_func = Just(twice)
    doubled_numbers = wrapped_twice_func.apply(numbers_list)
    print(f"List Functor 적용 결과: {doubled_numbers}") # 출력: List(2, 4, 6)
    ```
...
class Customer:
    ...
    def __repr__(self):
        return f"Customer(name='{self.name}', email='{self.email}', phone='{self.phone_number}', dob='{self.dob}')" # repr에 모든 필드 포함
...
# 출력:
# Alice: Success
# Bob: Email format is invalid
# Charlie: Phone number must start with +0
# David: Customer must be at least 16 years old
...
print(f"{customer1.name} (이메일/전화): {email_phone_validator(customer1).value}") # 출력: Alice (이메일/전화): Success
print(f"{customer3.name} (이메일/전화): {email_phone_validator(customer3).value}") # 출력: Charlie (이메일/전화): Phone number must start with +0
```
